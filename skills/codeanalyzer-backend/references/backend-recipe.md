...
(tree-sitter + Jedi); JS/TS → Node (ts-morph over the TS compiler, optional
Joern). Pick **one tool for the structural pass** and **one for resolution** — and
...
`-f/--format` (`json|msgpack`), level selection (`-a 1|2` like Java, or `--joern`-style
...
- **Tier 2 — Framework-based (a dedicated analysis engine):** WALA / Joern / SVF.
...
Layer Joern (`jssrc2cpg`) or SVF edges for the dynamic/dataflow cases the primary
checker misses, then **merge by `(source, target)`** with provenance union and weight
accumulation — the exact resolver∪framework merge pattern in `codeanalyzer-python`'s core. Gate it
...
(Python's `semantic_analysis/<framework>/` splits binary resolution, DB/driver, query execution, and
